...
class ATSEngine {
     ...
     */
    static analyzeResume(resumeText, jobDescription) {
        ...
        // Weight the scores
        const overallScore = (keywordScore * 0.4 +
            semanticScore * 0.35 +
            impactScore * 0.25);
    ...
    static calculateKeywordMatch(resumeText, jobDescription) {
        ...
        if (jobTokens.length === 0)
            return 0;
        const matchedCount = jobTokens.filter(token => resumeTokens.includes(token)).length;
        return matchedCount / jobTokens.length;
    }
    /**
     * Calculate semantic similarity using TF-IDF
     */
    static calculateSemanticSimilarity(resumeText, jobDescription) {
     ...
     */
    static calculateImpactScore(resumeText) {
        ...
        for (const line of lines) {
            ...
            if (line.trim().match(/^[•\-*]/)) {
                ...
                const hasImpactVerb = IMPACT_VERBS.some(verb => line.toLowerCase().includes(verb));
                if (hasImpactVerb)
                    lineScore += 0.5;
                // Check for metrics
                const hasMetrics = METRIC_PATTERNS.some(pattern => pattern.test(line));
                if (hasMetrics)
                    lineScore += 0.5;
     ...
     */
    static getKeywordAnalysis(resumeText, jobDescription) {
     ...
     */
    static generateSuggestions(resumeText, jobDescription, scores) {
        const suggestions = [];
        if (scores.keywordScore < 0.5) {
            const { missing } = this.getKeywordAnalysis(resumeText, jobDescription);
            suggestions.push(`Include more relevant keywords from the job description: ${missing.slice(0, 5).join(', ')}`);
        }
        if (scores.impactScore < 0.6) {
            suggestions.push('Add more quantifiable achievements with specific metrics (%, $, numbers)');
            suggestions.push(`Start bullet points with strong action verbs: ${IMPACT_VERBS.slice(0, 5).join(', ')}`);
        }
        if (scores.semanticScore < 0.5) {
            suggestions.push('Better align your experience descriptions with the job requirements');
        }
